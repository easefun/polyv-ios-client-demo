...
@implementation PvDanmuOperateView
...
- (void)p_addView {
    ...
    [self.sendBtn setTitle:@"发射" forState:UIControlStateNormal];
    [self addSubview:self.sendBtn];
    [self.sendBtn addTarget:self action:@selector(sendDanmuAction:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)sendDanmuAction:(UIButton *)btn {
    if ([self.deleagte respondsToSelector:@selector(sendDanmu:)]) {
        [self.deleagte sendDanmu:self.editContentTF.text];
    }
}
