...
#import "PvDanmuOperateView.h"

#define VIEW_OPERATE_HEIGHT    44

@interface PvDanmuSendView () <PvDanmuOperateViewDelegate, UITextFieldDelegate>

@property (nonatomic, strong) PvDanmuOperateView *danmuOperateV;
@property (nonatomic, strong) UIControl *blackOverlay;

@end

@implementation PvDanmuSendView
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [self.blackOverlay addTarget:self action:@selector(backAction) forControlEvents:UIControlEventTouchUpInside];

        
        self.danmuOperateV = [[PvDanmuOperateView alloc] initWithFrame:CGRectMake(0, self.frame.size.height - VIEW_OPERATE_HEIGHT, self.frame.size.width, VIEW_OPERATE_HEIGHT)];
...
#pragma mark - PvDanmuOperateViewDelegate
...
- (void)sendDanmu:(NSString*)danmu {
    if ([self.deleagte respondsToSelector:@selector(sendDanmu:info:)]) {
        [self.deleagte sendDanmu:self info:danmu];
    }
    [self backAction];
}
